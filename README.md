...
- Веб: `https://<домен>/mini-app/?web=1`. Приглашение: страница `https://<домен>/invite/<token>` (браузер, Telegram App, Telegram Web).
- Ссылка «Скопировать приглашение» собирается из **`APP_URL`**: `https://<APP_URL>/invite/<token>`. Задайте `APP_URL` на тот же публичный домен, с которого открывают приложение.
- Владелец команды может **исключить участника** в «Настройки → Команда»; участник может **покинуть команду** сам.
- Приглашения: срок **1 / 7 / 30 дней** или **без срока**; список активных ссылок и отзыв (владелец).
- После вступления в команду бот шлёт уведомление участнику и владельцу (если задан `APP_URL`).
- При открытии ссылки с `invite` в **браузере** веб-версии сессия сбрасывается и показывается вход через Telegram — иначе в этом браузере могла остаться cookie предыдущего пользователя, и вы бы видели **чужой** аккаунт. В Telegram mini app используется ваш Telegram-аккаунт (cookie веб-входа не нужен).
- В Telegram mini app: переключение в «Ещё → Команда и доступ», настройки команды в «Настройки» (участники, приглашения, выход).
- В командном режиме в истории операций видно, **кто внёс** запись.
- **Операции и переводы**: создание, **редактирование** и **удаление** (веб — колонка «Действия» в истории; TG — свайп влево в «Истории»).
- При **выходе** из веб-аккаунта сбрасываются cookie сессии, активный workspace и локальный выбор «личный / команда».
...
- В веб-версии: **Настройки → Журнал изменений** (отдельная страница; виден всем участникам команды).
- В **Истории** (веб, командный режим) у каждой операции кнопка **История изменений** — события только по этой записи.
- Для старых записей без `details` подставляются данные из текста события; новые события хранят сумму, валюту и название операции в `details` (миграция `009_audit_events_details.sql`).
