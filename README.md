...
- Ссылка «Скопировать приглашение» собирается из **`APP_URL`** на сервере (`https://<APP_URL>/mini-app/?invite=…`). Так в Telegram не попадает длинный URL вида `…tgWebApp…` из WebView. Задайте `APP_URL` на тот же публичный домен, с которого реально открывают приложение.
